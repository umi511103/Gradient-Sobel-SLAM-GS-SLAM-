...
void processScanLineRegions_surface(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, int startScanID, int endScanID, int half_window_size,
                                    double depth_threshold, double gradient_threshold, int intensity_threshold, int window_size,
                                    std::vector<pcl::PointXYZI>& planePoints) {

    for (int scanID = startScanID + half_window_size; scanID < endScanID; scanID++) {
        std::cout<<"this is scan id"<<scanID<<std::endl;
        for (std::size_t pointIdx = half_window_size; pointIdx < cloud->points.size() - half_window_size; pointIdx++) {
            float minZ = std::numeric_limits<float>::max();
            float maxZ = std::numeric_limits<float>::lowest();

            // Check along the scan line (using scanID and azimuth angle)
            for (int wx = -half_window_size; wx <= half_window_size; ++wx) {
                int idx = pointIdx + wx;
                if (cloud->points[idx].intensity == scanID) {
                    const pcl::PointXYZI& point = cloud->points[idx];
            ...
            if (maxZ - minZ <= depth_threshold) {
                const pcl::PointXYZI& point_left = cloud->points[pointIdx - half_window_size];
                const pcl::PointXYZI& point = cloud->points[pointIdx];
                const pcl::PointXYZI& point_right = cloud->points[pointIdx + half_window_size];

                double gradient_1 = std::abs(point_left.z - point.z);
                double gradient_2 = std::abs(point_right.z - point.z);
...
void processPointCloud_surface(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud, int half_window_size,
                               double depth_threshold, double gradient_threshold, int intensity_threshold, int window_size,
                               std::vector<pcl::PointXYZI>& planePoints) {
    int numThreads = 16;  // Number of threads to use
    std::vector<std::thread> threads;
    std::vector<std::vector<pcl::PointXYZI>> planePointsList(numThreads);

    int totalScanLines = cloud->height;
    int scanLinesPerThread = totalScanLines / numThreads;
    int remainingScanLines = totalScanLines % numThreads;
    int startScanID = 0;

    for (int i = 0; i < numThreads; ++i) {
        int endScanID = startScanID + scanLinesPerThread;
        if (i == numThreads - 1) endScanID += remainingScanLines;
        // Pass cloud by value (shared_ptr will manage the reference counting)
        threads.emplace_back(processScanLineRegions_surface, cloud, startScanID, endScanID, half_window_size, depth_threshold,
                             gradient_threshold, intensity_threshold, window_size, std::ref(planePointsList[i]));
        startScanID = endScanID;
    }
    ...
    for (const auto& points : planePointsList) {
        planePoints.insert(planePoints.end(), points.begin(), points.end());
    }
}
...
void LaserProcessingClass::pointcloudtodepth(pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in,
    ...
    // 使用原始的 3D 点云数据进行处理，不进行投影
    pcl::PointCloud<pcl::PointXYZI>::Ptr pc_original(new pcl::PointCloud<pcl::PointXYZI>());
    *pc_original = *pc_in; // 保留原始的3D点云数据                                           
    ...
    processPointCloud_surface( surf_first , half_window_size,
    depth_threshold,  gradient_threshold ,  intensity_threshold,  window_size,
    planePixels);
