...
void processPointCloudRegions_surface(const pcl::PointCloud<pcl::PointXYZI>::Ptr& surf_first, 
    ...
    for (int i = startIdx + half_window_size; i < endIdx - half_window_size; i++) {
        bool isPlanar = false;
        
        // Check depth values along the x-axis
        float minDepth = std::numeric_limits<float>::max();
        float maxDepth = std::numeric_limits<float>::min();

        for (int wx = -half_window_size; wx <= half_window_size; ++wx) {
            float depth = surf_first->points[i + wx].z;
            if (!std::isnan(depth)) {
        ...
        if (maxDepth - minDepth <= depth_threshold) {
            // If depth values are within threshold, check gradients along the y-axis
        ...
        if (!isPlanar) {
            // Check depth values along the y-axis
            float minDepth = std::numeric_limits<float>::max();
            float maxDepth = std::numeric_limits<float>::min();

            for (int wy = -half_window_size; wy <= half_window_size; ++wy) {
                float depth = surf_first->points[i + wy].z;
                if (!std::isnan(depth)) {
                    minDepth = std::min(minDepth, depth);
                    maxDepth = std::max(maxDepth, depth);
                }
            ...
            if (maxDepth - minDepth <= depth_threshold) {
                // If depth values are within threshold, check gradients along the x-axis
...
void LaserProcessingClass::pointcloudtodepth(pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in,
    ...
    double gradient_threshold = 2;

    std::vector<pcl::PointXYZI> planePixels;

    processPointCloud_surface(pc_in, half_window_size, depth_threshold, gradient_threshold, window_size, planePixels);
