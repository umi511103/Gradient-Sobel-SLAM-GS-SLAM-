...
void LaserProcessingClass::featureExtraction(pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, 
    ...
    // // 做Canny邊緣檢測
    // Canny(gray_cropped, gray_cropped, 140, 90);
    ...
    gray_cropped.copyTo(gray(cv::Rect(0, third_rows, gray_cropped.cols, gray_cropped.rows)));
...
void LaserProcessingClass::featureExtractionFromSector(const pcl::PointCloud<pcl::PointXYZI>::Ptr& pc_in, 
    ...
    for (int i = cloudCurvature.size()-1; i >= 0; i=i-1)
    {
        ...
        if(std::find(picked_points.begin(), picked_points.end(), ind)==picked_points.end()){
            ...
            //一個segment曲率前10大的都push進edge點 //0529
            if (largestPickedNum <= 20 && cloudCurvature[i].value > 0.1 && std::abs(pc_in->points[ind].y) >= 0.5){
                pc_out_edge->push_back(pc_in->points[ind]);
                point_info_count++;
            }
            //若超過20個之後的點曲率還是大於2也是push進edge點  //0529
            else if(cloudCurvature[i].value > 0.5 && std::abs(pc_in->points[ind].y) >= 0.5){
                pc_out_edge->push_back(pc_in->points[ind]);
                picked_points.push_back(ind);
            }
            else{
